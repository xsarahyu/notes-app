    ...
    return (<div className='flex items-center justify-between bg-purple-50 rounded-lg border border-gray-200 p-2 w-full'>
      <div className='flex items-center justify-between gap-5 w-full'>
        ...
        <div className='flex items-center gap-2'>
          <input id='time' type='number' min={0.5} step={0.5} className={`w-18 h-8 text-center text-slate-600 text-xs rounded border border-gray-200 focus:outline-none focus:border-transparent focus:ring-2 focus:ring-purple-300 focus:ring-opacity-50 ${isDone && 'pointer-events-none opacity-50'}`} value={time} onChange={handleTimeChange}/>
...
function MainTaskTable({ mainTask, subtasks }) {
    return (<>
      <thead>
        <tr>
          <th className={`flex items-center justify-between gap-5 py-4 px-3 text-slate-800 border rounded-md border-slate-200 bg-opacity-70 ${mainTask.priority === 'high'
            ? 'bg-red-100'
      ...
      {!!subtasks ? (subtasks.map((subtask) => {
            if (subtask.mainTaskName === mainTask.name) {
                ...
                <tr>
                  <td className={`flex items-center justify-between gap-4 py-2 px-3 text-slate-600 border rounded-md border-purple-100 bg-opacity-60 ${mainTask.priority === 'high'
                        ? 'bg-red-50'
                        : mainTask.priority === 'low'
                            ? 'bg-green-50'
                            : 'bg-yellow-50'}`}>
...
function SubtaskTable({ description, time }) {
    ...
    return (<>
      ...
      <span className={`leading-tight justify-self-start w-full text-slate-600 ${isDone ? 'line-through text-slate-500 opacity-50' : ''}`}>
        {description}
      </span>
